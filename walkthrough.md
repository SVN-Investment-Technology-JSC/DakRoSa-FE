...
- **Xem Trưởng đơn vị (Head)**: mỗi node hiển thị người đứng đầu (ví dụ Khối Kỹ thuật → Nguyễn Văn Tuấn). Đơn vị chưa có trưởng hiện dòng cam **"Chưa có trưởng đơn vị — bổ nhiệm"**, bấm vào là mở luôn bảng nhân sự.
- **Xem nhân sự của từng đơn vị**: mỗi thẻ giờ liệt kê **danh sách nhân sự** bên dưới phần trưởng đơn vị, kèm chức vụ của từng người. Trưởng đơn vị không bị liệt kê lại lần hai.
- **Quản lý nhân sự** — di chuột vào node, bấm biểu tượng 👥 (hoặc **"+ Thêm người"** trong danh sách). Bảng mở ra làm được 4 việc:
  - **Bổ nhiệm / đổi / gỡ Trưởng đơn vị** — chọn từ nhân sự sẵn có trong đơn vị hoặc từ toàn bộ danh bạ. Đây là điều kiện để gán vai trò cho **cả đơn vị** ở Ma trận RSACIE (xem mục 4.2).
  - **Thêm nhân sự vào đơn vị** — tìm theo họ tên hoặc email (**gõ không dấu vẫn ra**: "tuan" tìm thấy "Nguyễn Văn Tuấn"), chọn chức vụ, bấm thêm. Người đã ở trong đơn vị không hiện lại trong danh sách chọn.
  - **Tạo tài khoản mới** — nút **"Người chưa có tài khoản?"**: nhập họ tên / email / mật khẩu, hệ thống tạo tài khoản (mặc định quyền `approver` để họ xử lý được công việc) và **thêm luôn vào đơn vị đang mở** với chức vụ đã chọn.
  - **Gỡ nhân sự khỏi đơn vị** — biểu tượng 👤− ở cuối mỗi dòng. Gỡ người đang là trưởng khỏi danh sách **không** đồng thời gỡ ghế trưởng; dùng nút "Gỡ" ở mục Trưởng đơn vị cho việc đó.

> ⚠️ **Chưa hỗ trợ trong bản này**: đổi tên node, kích hoạt/vô hiệu hoá node, và **chuyển nhánh** (đổi đơn vị cha) chưa có nút trên giao diện — `PATCH /org-units/:id` đã có sẵn ở backend cho hai việc đầu. Xoá / vô hiệu hoá **tài khoản người dùng** cũng chưa có (chỉ tạo được).
...
- **Cột sổ ngang theo cây tổ chức**. Bấm vào tên cột (icon `+`) để sổ ra, bấm lại (icon `-`) để thu về. Mở rộng một **Đơn vị** cho ra: cột **Trưởng đơn vị** (chính là đơn vị đó) + một cột cho **từng nhân sự** + các **đơn vị con**.
  - Icon đầu mỗi cột cho biết đang ở lớp nào: 🏢 Đơn vị · 👤 Cá nhân.
  - **Chỉ có hai kiểu đích gán: ĐƠN VỊ hoặc CÁ NHÂN.** Không còn lớp "Chức vụ" — một tag gán cho chức vụ trỏ tới nhiều người cùng lúc nên khi sổ cột ra nó hiện lặp ở từng người, đọc như thể có nhiều tag khác nhau. Chức vụ vẫn còn, nhưng chỉ là thông tin nhân sự in phía trên tên người ở đầu cột.
...
- **Header gộp nhóm 2+ tầng**: khi sổ một đơn vị ra, tên đơn vị đó trở thành **tiêu đề nhóm nằm ngang bên trên**, bên dưới là các cột con. Giống hệt kiểu merge ô của bảng tính.
- **Không mất dữ liệu khi sổ ra**: vai trò đã gán cho cả đơn vị nằm ở cột **Trưởng đơn vị** sau khi sổ — vì gán cho đơn vị nghĩa là giao cho trưởng đơn vị. Không có cột giả "(Cả đơn vị)".
...
- **Gán vai trò ở bất kỳ lớp nào** — cột đang là gì thì gán thẳng vào đó:
  - Gán ở cột **Đơn vị** (hoặc cột **Trưởng đơn vị**) → giao cho **Trưởng đơn vị** đó.
  - Gán ở cột **Cá nhân** → giao **đích danh** người đó. Muốn 2 người cùng phải xử lý một bước thì gán chữ R cho cả 2 cột cá nhân (xem AND-logic ở mục 4.3).
  - **Đơn vị chưa có trưởng thì không gán cho cả đơn vị được.** Gán cho đơn vị nghĩa là giao cho trưởng, mà ghế đang trống thì không có ai nhận. Ô đó chỉ hiện `-` mờ, bấm vào sẽ báo rõ lý do và bảng chọn chữ cái bị ẩn. Cách xử lý: bổ nhiệm trưởng ở **Sơ đồ Tổ chức**, hoặc sổ cột ra và gán thẳng cho một cá nhân trong đơn vị đó (cá nhân **không** bị luật này chặn). Server cũng chặn lần nữa cho chắc.
...
> ℹ️ **Quản lý Chức vụ & Nhân sự**: danh mục chức vụ và danh sách nhân sự của từng đơn vị đã có API thật (`/positions`, `/org-units/:id/members`). Dữ liệu mẫu đã seed sẵn (Trưởng đơn vị / Nhân viên cho các đơn vị). Chức vụ dùng để hiển thị và sắp thứ tự cột, **không phải đích gán vai trò**. Giao diện quản trị riêng để thêm/sửa nhân sự thì **chưa làm** — hiện chỉnh qua seed hoặc gọi API trực tiếp.
...
- **Tạo đơn mới**: bấm **"Tạo Đơn / Yêu cầu Mới"**, chọn quy trình, chọn đơn vị Cấp 1 phụ trách, nhập tiêu đề/mô tả/mức ưu tiên/hạn hoàn thành — hệ thống tự sinh toàn bộ các bước và tự động phân công người phụ trách từng bước dựa trên Trưởng đơn vị đã gán ở Sơ đồ Tổ chức + tag RACI đã cấu hình ở Ma trận.
  - **Ô chọn quy trình chỉ hiện những quy trình bạn giữ chữ S (Đề xuất)** — không phải toàn bộ danh mục. Không giữ S ở đâu thì danh sách rỗng kèm lời nhắc đi xin gán chữ S. Server chặn lần nữa ở `POST /tasks` (trả **403**), nên không thể lách bằng cách gọi thẳng API.
  - Nhắc lại luật của chữ S (mục 4.3 bên dưới): gán **S cho một đơn vị** nghĩa là **mọi thành viên đơn vị đó, kể cả các tổ bên dưới** đều mở được đơn — khác với R/A/C/I/E vốn dồn về trưởng đơn vị.
  - Hai nguồn đơn do hệ thống tự sinh (**Luồng Thực thi** phái sinh, và **Lệnh công việc** từ phiếu bảo trì) không bị luật S chặn — chúng không có người đề xuất nào để kiểm.
...
- **"Tiến trình các bước" hiện thẳng tên người**, không phải tên đơn vị: `C: Nguyễn Văn Tuấn`, `R: Phạm Thị Hà, Lê Văn Nam`. Tên lấy từ những người **đã thực sự được phân công** lúc tạo đơn, nên đã tính cả escalation (kèm nhãn *xử lý thay*) lẫn uỷ quyền. Với đơn gán cho cả đơn vị, tên đơn vị vẫn được ghi sau dấu `—` để biết chữ cái đó đến từ ô nào của ma trận.
...
- **Siết đích gán vai trò + hiện tên người (4 sửa đổi theo phản hồi)**:
  1. **Bỏ hẳn cấp "Chức vụ" khỏi ma trận** — chỉ còn hai kiểu đích: **Đơn vị** (mặc định là trưởng đơn vị) và **Cá nhân**. Một tag gán theo chức vụ trỏ tới nhiều người cùng lúc, nên khi sổ cột ra nó hiện lặp ở từng người dưới dạng ô nét đứt, đọc như thể có nhiều tag khác nhau. Chức vụ vẫn còn trong dữ liệu nhân sự, chỉ không còn là đích gán. Chạy `npm run seed` sẽ **tự gỡ** các tag chức vụ cũ trong database.
  2. **Ô chọn quy trình khi tạo đơn chỉ hiện quy trình bạn giữ chữ S** (mục 4.3), chặn cả ở server.
  3. **"Tiến trình các bước" hiện tên người** thay vì tên đơn vị — `C: Nguyễn Văn Tuấn` (mục 4.3). Đơn tạo trước thay đổi này cũng hiện đúng, vì tên được dựng lại từ danh sách người đã phân công chứ không đọc chuỗi tóm tắt cũ.
  4. **Đơn vị chưa có trưởng không phải đích gán hợp lệ** (mục 4.2) — gán cho đơn vị nghĩa là giao cho trưởng, ghế trống thì không ai nhận. Gán cho **cá nhân** trong đơn vị đó thì vẫn được. Escalation (ghế trống lúc chạy tự đẩy lên cấp trên) **giữ nguyên** — đó là lưới an toàn khi trưởng nghỉ giữa chừng, không phải cách hợp thức hoá một ô cấu hình sai.
  - **Dữ liệu mẫu đổi theo**: bước 2 của `WF-CAPEX` nay gán R cho **2 cá nhân** của Tổ Backend (thay cho 1 tag chức vụ) — vẫn giữ nguyên phép thử AND-logic; bước 3 chuyển R từ **Tổ QA** (cố tình không có trưởng) sang **Ban Phát triển Phần mềm**. Chạy lại seed cũng tự gỡ mọi tag đang trỏ vào đơn vị chưa có trưởng.
- **Quản lý nhân sự ngay trên Sơ đồ Tổ chức** (mục 4.1): thêm `GET /users` + `POST /users` (đòi quyền `org.manage`) — trước đây không có endpoint danh sách người dùng nên mọi ô chọn người đều bế tắc. Nay bổ nhiệm/đổi/gỡ trưởng đơn vị, thêm/gỡ nhân sự, và tạo tài khoản mới đều làm được từ giao diện; mỗi thẻ đơn vị cũng liệt kê nhân sự của nó. Tìm kiếm người **không phân biệt dấu**. Đây là mảnh còn thiếu của luật "đơn vị chưa có trưởng thì không gán vai trò cho cả đơn vị" — giờ người dùng tự bổ nhiệm được thay vì phải gọi API tay.
  - **Lỗi thật phát hiện khi kiểm chứng**: `PATCH /org-units/:id` với `headUserId: null` **không gỡ được trưởng** — `OrgUnitsService.update` nạp entity kèm quan hệ `head` rồi `Object.assign` + `save()`, TypeORM thấy `unit.head` vẫn là object cũ nên tính lại khoá ngoại từ đó và ghi đè `null` trở về id cũ. Đúng cùng một cái bẫy đã gặp ở `WorkflowRequestsService.triage`. Đã sửa sang `repository.update()` với patch dựng tay, phân biệt rõ `null` (gỡ) với `undefined` (không đụng tới).
